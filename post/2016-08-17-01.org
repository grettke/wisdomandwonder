...
* Conclusions About CUSTOM_ID

- =CUSTOM_ID=
  - Is for human creation and use
  - Is for linking
  - *Must be unique*
  - =org-store-link= make this confusing because it creates both =ID= and
    =CUSTOM_ID= making it sound like at the end of the day, =ID= is the best way
    to maintain uniqueness.
- =org-id-link-to-org-use-id= really leans towards =CUSTOM_ID= always taking
  preference. Does it matter whether it is custom or not? Isn't it just more
  important that it exists? The list is silent on this anyway.
- Set up everything for =CUSTOM_ID= everywhere
